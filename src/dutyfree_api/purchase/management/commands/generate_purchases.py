...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--api-version", type=str, default="v1", help="API version (default: v1)")
        parser.add_argument("--count", type=int, default=5, help="Number of customers to generate (default: 5)")
    ...
    def handle(self, **options):
            ...
            if 200 <= response.status_code < 300:
                ...
                self.stderr.write(f"Failed with status {response.status_code}:\n{response.text}")
    ...
    def generate_customer(self):
        customers = {
            ...
            "city": fake.city(),
            "email": fake.email(),
            "purchases": [self.generate_purchase() for _ in range(random.randint(1, 3))]
        }
